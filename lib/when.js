...
export default class When extends Component {
  ...
  render () {
    ...
    return (
      <fieldset className='FromToWhen-When'>
        <label>
          <span>On <select name='days' defaultValue={days}>{daysOptions}</select></span>
          <span>from <select name='start' value={start} onChange={input => this.onStartChange(input)}>{startTimeOptions}</select></span>
          <span>to <select name='end' value={end} onChange={input => this.onEndChange(input)}>{endTimeOptions}</select></span>
        </label>
      </fieldset>
    )
  }
